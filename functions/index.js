...
exports.classifyInspirationSubcategories = onCall(
  ...
  async (request) => {
    ...
    // סימנים אמיתיים שנמצאו בבדיקה ידנית (2026-09-14): contentSummary שהוא
    // בעצם טקסט סירוב של Claude מבאג ישן ב-generateContentSummaries ("אני
    // לא יכול לתאר...", "התמלול אינו קשור לנושא..."). בלי הבדיקה הזו, הניסיון
    // הקודם "תיקן" את זה בכך שהכריח את המודל "לבחור את הכי קרוב" - וזה בדיוק
    // מה שגרם לתקלה שמאיה תפסה בפועל: שירים/תוכן לא-קשור נדחסו לתוך "איפור"
    // ותת-קטגוריות אחרות שנשמעו הכי "קרובות", כי המודל לא הורשה לסרב. הפתרון
    // הנכון הוא לדלג מראש על טקסט שכבר ידוע כסירוב/גיבריש, ולא לכפות בחירה.
    // "לא יכול לעזור..." (בלי "אני") פספס את הבדיקה הקודמת ונשאר בבסיס
    // הנתונים כאילו הוא contentSummary אמיתי - נתפס בבדיקה ידנית (2026-09-14).
    const REFUSAL_PATTERN = /^(אני לא יכול|אינני יכול|איני יכול|לא יכול|לא ניתן|I cannot|I appreciate|I'm unable)|אינו קשור|אינה קשור|לא קשור|אינו קריא|אינה קריאה|לא קריא|אינו מובן|אינה מובנת/;
    ...
    for (const doc of targets) {
      ...
      try {
        ...
        if (!text || text.trim().length < 10 || REFUSAL_PATTERN.test(text.trim())) {
        ...
        // "חייב/ת לבחור, גם אם לא מושלם" (הניסוח הקודם) גרם למודל לדחוס תוכן
        // לא-קשור (שירים, גיבריש) לתוך התת-קטגוריה הכי "קרובה" בצליל - זה
        // בדיוק מה שמאיה תפסה בפועל (טיפולי פנים תחת "איפור"). "לא_רלוונטי"
